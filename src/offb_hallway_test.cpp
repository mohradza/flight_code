...
int main(int argc, char **argv)
{
    ...
    geometry_msgs::PoseStamped hallway_start_pose;
    geometry_msgs::PoseStamped hallway_end_pose;
    ...
    // Set the takeoff pose
    takeoff_pose = home_pose;
    takeoff_pose.pose.position.y = 0.0;
    takeoff_pose.pose.position.x = 0.0;
    takeoff_pose.pose.position.z = 1.0;

    // Set the first maneuver point
    hallway_start_pose = takeoff_pose;
    hallway_start_pose.pose.position.x = -2.0;

    // Set the second manuever point
    hallway_end_pose = hallway_start_pose;
    hallway_end_pose.pose.position.x = 2.0;
    ...
    while(ros::ok()){
        ...
        if(man1 && !man2){
            if(ros::Time::now() - last_request < ros::Duration(5.0)){
              ...
              ROS_INFO("Sending Pose: hallway_start");
              pose = hallway_start_pose;
              pose.header.stamp = ros::Time::now();
              man2 = true;
              last_request = ros::Time::now();
              record_data.data = false;
        ...
        // Moving to Hallway Start
        if(man2 && !man3){
            if(ros::Time::now() - last_request < ros::Duration(7.0)){
                pose.header.stamp = ros::Time::now();
            } else {
              ROS_INFO("Sending Pose: hallway_end");
              pose = hallway_end_pose;
              pose.header.stamp = ros::Time::now();
              man3 = true;
              last_request = ros::Time::now();
            }
        ...
        // Moving to Hallway End
        if(man3 && !man4){
            if(ros::Time::now() - last_request < ros::Duration(15.0)){
                pose.header.stamp = ros::Time::now();
            } else {
                ROS_INFO("Sending Pose: Takeoff Position");
                pose = takeoff_pose;
                pose.header.stamp = ros::Time::now();
                man4 = true;
                last_request = ros::Time::now();
        ...
        if(landed && current_state.armed){
            if(arming_client.call(disarm_cmd) && disarm_cmd.response.success){
                ROS_INFO("Vehicle disarmed, exiting program");
                return 0;
             }
