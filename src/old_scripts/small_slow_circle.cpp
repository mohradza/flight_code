...
int main(int argc, char **argv)
{
    ...
    bool sitl = true;
    float pi_trunc = 3.14159;
    bool manuever_start = false;
    // MAIN CONTROL LOOP
    while(ros::ok()){
        ...
        if(man1 && !man2){
            if(ros::Time::now() - last_request < ros::Duration(5.0)){
              ...
              ROS_INFO("Sending Circle positions");
        ...
        if(man2 && !man3){
            ...
            d = ros::Time::now() - man_start_time;
            
            pose.pose.position.x = .5*cos(2*pi_trunc*(5.0-d.toSec())/5.0);
            pose.pose.position.y = .5*sin(2*pi_trunc*(5.0-d.toSec())/5.0);

            if(ros::Time::now() - last_request > ros::Duration(20.0)){
                 ROS_INFO("Circle is Finished, returning to takeoff position...");
                 last_request = ros::Time::now();
                 pose = takeoff_pose;
                 pose.header.stamp = ros::Time::now();
                 man3 = true;
