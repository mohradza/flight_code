...
#include <flight_code/FlowRingOutMsg.h>
#include <iostream>
...
private:
    ...
    float yaw_rate_cmd;
    float min_threshold;
    flight_code::YawRateCmdMsg yaw_rate_cmd_msg; 
    int sign;
    float gamma_arr[60];
    flight_code::FlowRingOutMsg current_flow;

public:
    SubscribeAndPublish(){
        // Create the yaw rate command publisher

        pub_ = nh_.advertise<flight_code::YawRateCmdMsg>
             ("/yaw_cmd_out", 10);
        // Create flow subscriber     
        sub_ = nh_.subscribe<flight_code::FlowRingOutMsg>
    ...
    void flow_cb(const flight_code::FlowRingOutMsg::ConstPtr& msg){
